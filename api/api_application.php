 ...
 */
$aColumns = array('id_aplikasi','ip', 'nama_client','uuid','status');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "id_aplikasi";

/* DB table to use */
$sTable = "manajemen_aplikasi";
...
while ($aRow = mysql_fetch_array($rResult)) {
		$row	 		= array();     
		$id_aplikasi	= $aRow['id_aplikasi'];
		$ip				= $aRow['ip'];
		$nama_client	= $aRow['nama_client'];
		$uuid			= $aRow['uuid'];
		$status			= $aRow['status'];
		if($status=='1'){
			$text_confirm	= "Aktif";
		}else{
			$text_confirm	= "Tidak Aktif";       
		}
		$row[]  		= $ip;
        $row[] 			= $nama_client;
        $row[]  		= $uuid;
